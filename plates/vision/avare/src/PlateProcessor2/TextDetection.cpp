...
void renderComponentsWithBoxes (IplImage * SWTImage, std::vector<std::vector<Point2d> > & components,
    ...
    cv::Mat outputMat = cv::Mat (output);
    cv::Mat outTempMat = cv::Mat (outTemp);
    cvConvertScale(outTemp, out, 255, 0);

    cv::cvtColor (outMat, outputMat, CV_GRAY2RGB);
...
void renderChainsWithBoxes (IplImage * SWTImage,
    ...
    IplImage * out =
            cvCreateImage ( cvGetSize ( output ), IPL_DEPTH_8U, 1 );
    cvConvertScale(outTemp, out, 255, 0);
    cv::Mat outMat (out); cv::Mat outputMat (output); cv::cvtColor (outMat, outputMat, CV_GRAY2RGB);
    cvReleaseImage ( &out );
    cvReleaseImage ( &outTemp);
...
IplImage * textDetection (IplImage * input, bool dark_on_light)
{
    ...
    cv::Mat inputMat (input); cv::Mat grayImageMat (grayImage); cv::cvtColor ( inputMat, grayImageMat, CV_RGB2GRAY );
    // Create Canny Image
    double threshold_low = 175;
    double threshold_high = 320;
    IplImage * edgeImage =
            cvCreateImage( cvGetSize (input),IPL_DEPTH_8U, 1 );
    cv::Mat edgeImageMat (edgeImage); cv::Canny(grayImageMat, edgeImageMat, threshold_low, threshold_high, 3) ;
...
//    cv::Mat gaussianImageMat (gaussianImage); cv::Smooth( gaussianImage, gaussianImage, CV_GAUSSIAN, 5, 5);
    cv::Mat gaussianImageMat (gaussianImage); cv::GaussianBlur( gaussianImageMat, gaussianImageMat, cv::Size(5,5), 0);
    IplImage * gradientX =
            cvCreateImage ( cvGetSize ( input ), IPL_DEPTH_32F, 1 );
    IplImage * gradientY =
            cvCreateImage ( cvGetSize ( input ), IPL_DEPTH_32F, 1 );
    ...
    cv::Mat gradientXMat (gradientX); 
    cv::Sobel(gaussianImageMat, gradientXMat , CV_32F, 1, 0, CV_SCHARR);

    //cvSobel(gaussianImage, gradientY , 0, 1, CV_SCHARR);
    cv::Mat gradientYMat (gradientY); cv::Sobel(gaussianImageMat, gradientYMat , CV_32F, 0, 1, CV_SCHARR);

    //cvSmooth(gradientX, gradientX, 3, 3);
    cv::GaussianBlur( gradientXMat, gradientXMat, cv::Size(3,3), 0);

    //cvSmooth(gradientY, gradientY, 3, 3);
    cv::GaussianBlur( gradientYMat, gradientYMat, cv::Size(3,3), 0);
    ...
    cv::Mat output4Mat (output4); cv::Mat output5Mat (output5); cv::cvtColor (output4Mat, output5Mat, CV_GRAY2RGB);
...
IplImage * loadByteImage ( const char * name )
{
  ...
  cv::Mat imageMat (image); cv::cvtColor (imageMat, imageMat, CV_BGR2RGB);
...
int mainTextDetection ( const char* imgFileName, 
                        const char* outFileName,
                        const int darkOnLight )
{
  IplImage * byteQueryImage = loadByteImage ( imgFileName );
  if ( !byteQueryImage )
  {
  ...
  cvSaveImage ( outFileName, output );
