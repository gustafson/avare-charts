...
for a in `ls */*/*tif`; do
    ...
    gdal_translate -of vrt -expand rgb ${lat}/${lon}/${tif} ${MERGE}/${lat}_${lon}_rbg.vrt
    gdalbuildvrt -addalpha -srcnodata '0 0 0' -srcnodata '255 255 255' ${MERGE}/${lat}_${lon}_rbga.vrt ${MERGE}/${lat}_${lon}_rbg.vrt 
    gdalwarp -of vrt -t_srs 'EPSG:900913' ${MERGE}/${lat}_${lon}_rbga.vrt ${MERGE}/${lat}_${lon}_warped.vrt
    gdal_translate -of vrt -projwin_srs WGS84 -projwin ${west} ${north} ${east} ${south} ${MERGE}/${lat}_${lon}_warped.vrt ${MERGE}/${lat}_${lon}_cropped.vrt
done
